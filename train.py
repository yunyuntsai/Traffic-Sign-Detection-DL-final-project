...
def run_training():
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		for epoch in range(NUM_EPOCH):
			...
			last_time = time.time()
			if SAVE_MODEL  and epoch>0 and (epoch+1)%10==0:
                                # Save model
                                save_path = saver.save(sess, MODEL_SAVE_PATH+ str(epoch+1))
                                print('Trained model saved at: %s' % save_path)
                                # Also save accuracy history
                                loss_path = 'lost_history2_'+str(epoch+1)+'.p'
                                print('Loss history saved at %s'%loss_path)
