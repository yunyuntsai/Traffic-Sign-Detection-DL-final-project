...
def generate_output(input_files, mode):
    ...
    if mode == 'image' or mode == 'video' or mode == 'demo':
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        #logging.critical('开始加载已训练模型 %s' MODEL_SAVE_PA100))
        saver.restore(sess, MODEL_SAVE_PATH+str(100))
        ...
        if mode == 'image':
            ...
            print('Demo mode: Running inference on images in TestImage/')
            image_files = os.listdir('sample_images/')
            l = []
            print("-" * 30)
            LINE = []
            for image_file in image_files:
                   print('Running inference on TestImage/%s' % image_file)
                   # image_orig = np.asarray(Image.open('TestImage/' + image_file))
                   image_orig = Image.open('TestImage/' + image_file)
                   image , a , l = run_inference(image_orig, model, sess, mode, sign_map)
                   print("######")
                   print(a)
                   print(l)
                   print(len(l))
                   ...
                   head, tail = os.path.split(image_file)
                   #plt.imsave('./inference_out/%s'% tail, image)
                   #print('save to inference_out/ %s' %tail)
                   plt.imshow(image)
                   plt.show()
                   print("-"*30)
                   coords = []
                   ...
                   if( len(l) > 0 ):
                        ...
                        for i in range(len(l)):
                             ...
                             sign_name =  l[i]
                             coords = a[i]  
                             print("Sign_name: %s"%sign_name)
                             print("coords : %s" %str(coords[0]))
                             line1 = ";" + sign_name + ";" + str(coords[0]) + ";" +str(coords[1]) +";" +str(coords[2])+ ";" +str(coords[3])                        
                             line += line1
                             print ("CSV : %s"%line)
                        LINE.append(line)
                             ...
                             LINE.append(line)	   
            print(LINE)   
            #f =  open('Detection2_Epoch_100.csv', 'w')        
